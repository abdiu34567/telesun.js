...
var connectBot = class Telesun {
  ...
  temporaryMemory(instance) {
    ...
    Utils.ensureValidService(
      instance,
      this.EXPECTED_METHODS.cacheService,
      "CacheService"
    );
    ...
    this.configs.TCache = new TemporaryCacheManager(
      this.memoryInstances.temporaryMemory
    );
    this.configs.TSession = new TemporarySessionManager(
      this.memoryInstances.temporaryMemory
    );
  ...
  permanentMemory(instance) {
    if (!instance) {
      throw new Error(ERRORS.PROPERTY_SERVICE_INSTANCE_NULL);
    }

    // Use Utils class to check if the instance is valid
    Utils.ensureValidService(
      instance,
      this.EXPECTED_METHODS.propertyService,
      "PropertyService"
    );
    ...
    this.configs.PCache = new PermanentCacheManager(
      this.memoryInstances.permanentMemory
    );
    this.configs.PSession = new PermanentSessionManager(
      this.memoryInstances.permanentMemory
    );
    return this; // Enable chaining
  }
    ...
    const next = () => {
      while (index < this.handlers.length) {
        ...
        if (type === "use") {
          ...
          type === "on" &&
          this.defaultManagers.updateManager.updateType() === data
        ) {
          // 'on' handler, execute if the update type matches
          return middleware(ctx, next);
        } else if (type === "stage" && ctx.getStage() === data) {
          // Handler type conditions are met, execute middleware
          return middleware(ctx, next);
        } else if (
          Utils.handlerConditions[type] &&
  ...
  polling(options = {}) {
    ...
    var lastOffset =
      PropertiesService.getUserProperties().getProperty("lastOffset");
    lastOffset = lastOffset ? parseInt(lastOffset, 10) : 0; // Default to 0 if not set

    const updates = Utils.apiRequest(this.botToken, "getUpdates", {
  ...
  deleteWebhook({ drop_pending_updates } = {}) {
    const res = Utils.apiRequest(this.botToken, "deleteWebhook", {
      drop_pending_updates,
    });
